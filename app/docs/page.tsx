...
const DOCS_TITLE = "Help & Documentation — VaultMTG";
const DOCS_DESCRIPTION =
  "Installation guide, pairing walkthrough, troubleshooting, and FAQ for the VaultMTG MTG Arena companion app.";
...
const FAQ_ITEMS: Array<{ q: string; a: string }> = [
  ...
  {
    q: "How do I pair VaultMTG with MTG Arena?",
    a: "Open VaultMTG and click \"Pair with Arena\" in the setup screen. Sign in with your Clerk account when prompted. Then launch MTG Arena — the daemon will detect the game and complete pairing automatically. The tray icon turns sapphire when pairing succeeds.",
  },
  {
    q: "My collection is not syncing. What do I do?",
    a: "Make sure MTG Arena is running and that you have completed a game or opened your collection in Arena after launching VaultMTG. The sync runs after each Arena session. If data still does not appear after 10 minutes, try signing out of VaultMTG and signing back in.",
  },
  {
    q: "How do I report a bug?",
    a: "The fastest path is the in-app bug report button (Help menu → Report a Bug). It attaches diagnostic logs automatically. For Discord users: post in #bugs with your OS, VaultMTG version, and the steps that caused the problem. Each bug report gets a GitHub issue filed and you will be notified with the issue link.",
  },
  {
    q: "Does VaultMTG work on Windows?",
    a: "Yes. VaultMTG supports macOS (Apple Silicon and Intel) and Windows (x64). The Windows build is unsigned during the current closed beta — Windows Defender or SmartScreen may show a warning. Click \"More info → Run anyway\" to proceed. Signing is in progress and will ship before public launch.",
  },
  {
    q: "Can I use VaultMTG on multiple devices?",
    a: "Your account and collection data are tied to your Clerk account and sync across devices. Each device needs the VaultMTG daemon installed and paired. Your collection history, match history, and draft ratings are available on every signed-in device.",
  },
  {
    q: "How do I uninstall VaultMTG?",
    a: "On macOS: drag VaultMTG.app from Applications to Trash, then remove the login item in System Settings → General → Login Items. On Windows: use Add or Remove Programs and uninstall \"VaultMTG\". Uninstalling does not delete your account data; sign in again on a new device to restore it.",
  },
];
...
const TOPICS: Array<{ id: string; marker: string; heading: string; body: string }> = [
  {
    ...
    body: "Download the VaultMTG installer for your platform from the download page. macOS and Windows (x64) are supported. The installer places a small background daemon and a tray icon on your system. MTG Arena must already be installed — VaultMTG reads Arena data locally and never contacts Wizards servers directly.",
  },
  {
    id: "pairing",
    marker: "§ 07.II",
    heading: "Pairing with MTG Arena",
    body: "After installing, open VaultMTG and follow the pairing wizard. Sign in with your account, then launch MTG Arena. The daemon detects the running game automatically. A successful pairing turns the tray icon sapphire. Pairing is per-device — if you install on a second machine, run the wizard again.",
  },
  {
    id: "troubleshoot",
    marker: "§ 07.III",
    heading: "Troubleshooting",
    body: "If VaultMTG is not reading data: (1) confirm MTG Arena is running in the foreground — not minimised to tray; (2) check that the VaultMTG daemon is running (tray icon present); (3) complete at least one game or collection browse in Arena after launch; (4) sign out and back in if the issue persists. For crashes or data-loss bugs, use the in-app bug report (Help → Report a Bug) which attaches logs automatically.",
  },
  {
    id: "data-sync",
    marker: "§ 07.IV",
    heading: "Collection & Match Data",
    body: "VaultMTG syncs your MTG Arena collection, match history, and draft history after each Arena session. Data is tied to your account and available on every signed-in device. Collection sync requires at least one Arena session after initial pairing. Match history includes all game modes tracked by the daemon. Historical data from before installation is not recoverable.",
  },
  {
    id: "uninstall",
    marker: "§ 07.V",
    heading: "Uninstalling",
    body: "On macOS: drag VaultMTG.app to Trash and remove the login item in System Settings → General → Login Items. On Windows: use Add or Remove Programs and uninstall \"VaultMTG\". Your account data is preserved in the cloud — sign in on any new device to restore your history.",
  },
];
      ...
      >
        <div style={{ margin: "0 auto", maxWidth: 860 }}>
          ...
          >
            ...
            to common questions — everything you need to get VaultMTG running.
          </p>
